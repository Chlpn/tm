...
class SwipeCard(models.TransientModel):
    ...
    def swipe(self):
        ...
        vals = {
            ...
            'cost_to_commission': (self.rec_amount * 'cost_percentage' / 100.0),
            'parent_percentage': cc_payment.machine_name.parent_name.cost_percentage or 0.0,
            'cost_to_parent': (self.rec_amount * 'parent_percentage' / 100.0),
            'margin': 'commission' - 'cost_to_commission',
            'cash_paid_customer': 0.0,
            'amount_to_customer': self.rec_amount - 'commission',

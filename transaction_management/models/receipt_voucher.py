...
class ReceiptVoucher(models.Model):
    ...
    partner_id = fields.Many2one('res.partner', string="Received From", required=True)
    ...
    _sql_constraints = [('name_uniq', 'unique (name)', 'Cash receipt voucher must be unique !')]
    _defaults = {'name': lambda self, cr, uid, context: 'Receipt'}
    ...
    def print_cash(self):
        return {
            ...
            'report_name': 'advanced_accounting.cash_receipt_report'
        }

...
class TransMaster(models.Model):
    ...
    def post(self):
            ...
            line_ids = [
                    (0, 0,
                     {'journal_id': journal_id, 'account_id': post_account,
                      'name': self.transaction_no+"/"+self.machine_name.name+"/"+self.amount_to_swipe,'partner_id': self.machine_name.rented_from.id,
                      'amount_currency': 0.0, 'debit': self.amount_to_swipe - self.cost_to_commission}),
                    (0, 0, {'journal_id': journal_id, 'account_id': self.machine_name.cost_ac.id, 'name':self.transaction_no+"/"+self.machine_name.name+"/"+self.amount_to_swipe,
                            'amount_currency': 0.0, 'debit': self.cost_to_commission}),
                    (0, 0, {'journal_id': journal_id, 'account_id': self.machine_name.cash_ac.id,
                            'name':self.transaction_no+"/"+self.machine_name.name+"/"+self.amount_to_swipe,
                    ...
                    (0, 0, {'journal_id': journal_id, 'account_id': self.customer.property_account_receivable_id.id,
                            'name': self.transaction_no+"/"+self.machine_name.name+"/"+self.amount_to_swipe,'partner_id': self.customer.id,
        ...
        vals = {
            'journal_id': journal_id,
            'ref': self.transaction_no+"/"+self.machine_name.name+"/"+self.amount_to_swipe,
        ...
        if self.machine_name.rent_again & self.env['res.users'].has_group('transaction_management.group_trans_admin'):
            ...
            rline_ids = [
                ...
                (0, 0, {'journal_id': rjournal_id, 'account_id': parent_account,
                        ...
                        'amount_currency': 0.0, 'debit': float_round(self.amount_to_swipe - self.cost_to_parent,precision_digits=2)}),

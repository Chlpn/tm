...
class TransMaster(models.Model):
    ...
    def compute_cbal(self):
        ...
        self.env.cr.execute(
            """select sum(debit-credit) from account_move_line left join account_move on account_move_line.move_id=account_move.id where account_id=2 and account_move_line.partner_id=8 and  account_move.state='posted' group by account_id""")

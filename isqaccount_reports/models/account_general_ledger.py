...
class report_account_general_ledger(models.AbstractModel):
        ...
        for account in sorted_accounts:
            ...
            if account in context['context_id']['unfolded_accounts'] or unfold_all:
                ...
                domain_lines.append({
                    ...
                    'name': _('Total '),
                    'footnotes': self.env.context['context_id']._get_footnotes('o_isqaccount_reports_domain_total', account.id),
                    'columns': ['', '', '', amount_currency, self._format(debit), self._format(credit), self._format(balance)],
                    'level': 1,
                })
                if too_many:
    ...
    def get_report_type(self):
        return self.env.ref('isqaccount_reports.account_report_type_general_ledger')

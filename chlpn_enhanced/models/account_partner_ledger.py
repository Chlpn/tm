...
class ReportPartnerLedger(models.AbstractModel):
        ...
        for partner in sorted_partners:
            ...
            if partner in context['context_id']['unfolded_partners'] or unfold_all:
                ...
                domain_lines.append({
                    ...
                    'footnotes': self.env.context['context_id']._get_footnotes('o_chlpn_enhanced_domain_total', partner.id),
                    'columns': ['', '', '', '', self._format(debit), self._format(credit), self._format(balance)],
                    'level': 1,
                })
                if too_many:
    ...
    def get_report_type(self):
        return self.env.ref('chlpn_enhanced.account_report_type_date_range_no_comparison')

...
class ReportAccountFinancialReport(models.Model):
    ...
    def create_action_and_menu(self, parent_id):
        client_action = self.env['ir.actions.client'].create({
            ...
            'context': {
                'url': '/chlpn_enhanced/output_format/financial_report/' + str(self.id),
    ...
    def get_report_type(self):
        return self.env.ref('chlpn_enhanced.account_report_type_' + self.report_type)

    def get_template(self):
        return 'chlpn_enhanced.report_financial'
...
class AccountFinancialReportLine(models.Model):
    ...
    def _get_with_statement(self, financial_report):
        ...
        if self.code not in ('CASHSTART', 'CASHEND') \
          and (financial_report == self.env.ref('chlpn_enhanced.account_financial_report_cashsummary0') or self.env.context.get('cash_basis')):
            ...
            if line in context.unfolded_lines or line.show_domain == 'always':
                ...
                if domain_ids:
                    lines.append({
                        'id': line.id,
                        'name': _('Total') + ' ' + line.name,
                        'type': 'o_chlpn_enhanced_domain_total',
                        'level': 1,
                        'footnotes': context._get_footnotes('o_chlpn_enhanced_domain_total', line.id),

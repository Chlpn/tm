...
class ReceiveACheque(models.TransientModel):
    ...
    def receive_cheque(self):
        ...
        model, journal_id = ir_model_obj.get_object_reference('cheque_management', 'cheque_receive_journal')
        interim_account = self.env['cheque.config.settings'].sudo().search([], order='id desc', limit=1)
        ...
        line_ids = [
            (0, 0,
             {'journal_id': journal_id, 'account_id': self.partner_account_id.id,
              'name': self.name, 'partner_id': self.commercial_partner_id.id,
            ...
            (0, 0, {'journal_id': journal_id, 'account_id': interim_account.interim_account_id.id, 'name': self.name,
                    'amount_currency': 0.0, 'partner_id': self.commercial_partner_id.id, 'debit': self.amount})
        ]
        values = {
        ...
        return {
            ...
            'name': 'Receive Cheque Master',
            'view_mode': 'tree,form',
            'view_type': 'form',
            'res_model': 'receive.cheque.master',
            'context': 'None'
        }
